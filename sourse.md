...
# 走失人口信息发布模块
走失人口信息发布模块是本平台重要的信息来源（未来本平台还会从其他平台导入数据，同时本平台的数据也会输出给有公信力的平台）。这个模块十分重要，功能入口要放在最显眼的位置，而且采集信息的流程要尽可能的简单流畅，以降低用户的学习成本；同时在采集信息时要尽可能的全面，而且要设计好采集模型，以支持条件检索和机器自动匹配处理，大大提高用户检索信息的效率；在采集用户将要发布的信息完成之后，在真正发布之前，系统要将用户的信息模型化，去和数据库比对，然后反馈给用户最接近他将要发布的的若干个条目，让用户确认自己要找的人是否就在其中。以提高寻人的成功率。
